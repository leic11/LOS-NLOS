...
\section{引言}
    ...
    针对上述问题，本文采用基于时空交叉注意力（Spatio\-temporal Cross\-Attention, STCA）的 NLOS 信号检测方法，该方法基于文献~\cite{zeng2024spatiotemporal}的工作。该方法的核心设计理念是：GNSS 信号同时包含空间分布特性（同一历元内多颗卫星的几何构型）和时间演化规律（信号质量随时间的动态变化），同时建模这两个维度，有助于实现对 NLOS 信号的准确判别。此外，引入可学习的稀疏表示层，通过非线性激活函数增强融合特征的判别能力。
