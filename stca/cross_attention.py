...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            query_dim: Query的输入维度 (如果为None，则使用embed_dim)
            kv_dim: Key和Value的输入维度 (如果为None，则使用embed_dim)
        """
    ...
    def forward(self, query, key, value, return_attention_weights=False):
        ...
        Args:
            query: 查询张量 (batch, 1, query_dim) - 时间特征 h_t
            key: 键张量 (batch, num_satellites, kv_dim) - 空间特征 h_s
            value: 值张量 (batch, num_satellites, kv_dim) - 空间特征 h_s
