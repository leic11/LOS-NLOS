...
使用方式：
    # 使用默认路径（自动读取 train/default_config.json）
    python -m eval.eval_static

    # 指定配置文件、数据、模型和输出路径
    python -m eval.eval_static \
        --config train/default_config.json \
        --data static_processed_indomain.npz \
        --model outputs/stca/final_model.pth \
        --output outputs/stca

    # 命令行参数说明：
    # --config  : 训练配置文件路径（默认 "train/default_config.json"）
    # --data    : 预处理后的 .npz 文件路径
    # --model   : PyTorch 模型权重文件路径
    # --output  : 输出目录路径
"""
import os
import sys
...
# Add stca dir to path for imports
...
def load_model(model_path: str, input_dim: int, device: torch.device, config: dict = None):
    ...
    # Create model instance: config from default_config.json
    model = STCAModel(
        input_dim=input_dim,
...
def prepare_test_loader(data, batch_size: int = 64, device: torch.device = None):
    ...
    if "X_test_spatial" in data and "X_test" in data:
...
def main(args):
    ...
    npz_path = args.data or str(script_dir / config.get(f"processed_npz_{split_mode}", f"static_processed_{split_mode}.npz"))
    # Default model path uses split_mode in filename
    default_model_name = f"final_model_{split_mode}.pth"
    model_path = args.model or str(script_dir / config.get("output_dir", "outputs/stca") / default_model_name)
    output_dir = args.output or str(script_dir / config.get("output_dir", "outputs/stca"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default=None,
                       help="Path to training config JSON file (default: train/default_config.json)")
