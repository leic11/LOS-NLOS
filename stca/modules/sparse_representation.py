...
class SparseRegularizer(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            embed_dim: 特征维度，应与交叉注意力输出维度一致（论文中为 64）
        """
        ...
        # 减小 b2 初始值以增大特征通过率
        self.b2_raw = nn.Parameter(torch.ones(embed_dim) * 0.5)  # softplus(0.5) ≈ 0.97
        self.b1_raw = nn.Parameter(torch.ones(embed_dim) * -2.0)  # sigmoid(-2) ≈ 0.12, b1 ≈ 0.12 * b2 ≈ 0.12
...
class SparseRepresentation(nn.Module):
    ...
    Input: (batch_size, fusion_out_dim) - 来自交叉注意力的融合特征
    Output: (batch_size, sparse_embed_dim) - 投影后的特征
    """

    def __init__(
        self,
        embed_dim: int = None,          # fusion_out_dim，输入维度
        sparse_embed_dim: int = None,   # 稀疏嵌入维度，输出维度
    ):
        """
        Args:
            embed_dim: 输入特征维度（融合特征的维度）
            sparse_embed_dim: 稀疏嵌入维度，输出特征维度
        """
        super(SparseRepresentation, self).__init__()
        self.embed_dim = embed_dim if embed_dim is not None else SPARSE_EMBED_DIM
        self.sparse_embed_dim = sparse_embed_dim if sparse_embed_dim is not None else SPARSE_EMBED_DIM

        # 线性投影层
        self.projection = nn.Linear(self.embed_dim, self.sparse_embed_dim)
        self.layer_norm = nn.LayerNorm(self.sparse_embed_dim)
